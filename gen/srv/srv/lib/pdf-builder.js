...
// Renders a label: value pair — bold label, value on same or next line
const labelValue = (label, value) => ({
  text: [{ text: `${label}: `, bold: true }, value || ''],
  margin: [0, 2, 0, 2],
});

// Renders all fields from a section config against the data object
...

    {
      table: {
        ...
        body: [
          ...
          ...(data.finanzpositionen ?? []).map(({ nummer, bezeichnung }) => [
            nummer ?? '', bezeichnung ?? '',
          ]),
        ],
      },
